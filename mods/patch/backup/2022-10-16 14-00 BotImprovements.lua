...
local world_direction_to_local_direction = function(unit, world_vector)
	...
	local rot = Unit.local_rotation(unit, 0)	-- Vernon: this should be a "flat" rotation
...
--]]
	...
	-- Vernon: choose one from back, left or right that aligns with the offset the most
...
Mods.hook.set(mod_name, "PlayerBotBase._enemy_path_allowed", function (func, self, enemy_unit)
	...
	if	(breed == Breeds.skaven_ratling_gunner and get(me.SETTINGS.NO_CHASE_RATLING)) or
		...
		return false
	--edit--
	-- Vernon TODO: add map geometry conditions and ogre HP conditions to ogre fight
...
Mods.hook.set(mod_name, "BTBotMeleeAction._update_engage_position", function(func, self, unit, blackboard, dt, t)
	...
	--edit--
		...
		elseif distance_deviation < -0.1 and breed and breed.name ~= "skaven_slave" and breed.name ~= "skaven_clan_rat" then
...
Mods.hook.set(mod_name, "BTAdvanceTowardsPlayersAction._calculate_trajectory_to_target", function(func, self, unit, world, blackboard, action)
	...
	if hit then
		...
		-- blackboard.throw_globe_data.target_position = Vector3Box()
