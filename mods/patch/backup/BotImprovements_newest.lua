...
* Fixed cras regarding missing unit_alive function in newly implemented hook.
...
local d_data_create_default = function(owner)
	...
	--edit--
	d.defense.dodge_count_left		= 2
	d.defense.is_dodging			= false
	--edit--
...
local d_data_update = function(d)
	...
	if bot_settings.menu_settings.reduce_ambient_shooting and not d.objective_defence.enabled then
		if not may_shoot_ambients then
			...
			may_shoot_ambients = aggroed_trash > 11 or aggroed_SV > 2 or #d.enemies.bosses > 0
			--edit--
		else
...
Mods.hook.set(mod_name, "PlayerBotBase._update_target_enemy", function(func, self, dt, t)
	...
	for _,loop_unit in pairs(prox_enemies) do
		...
		if loop_dist and loop_blackboard and loop_blackboard.target_unit then
			if ((loop_breed.name == "skaven_clan_rat" or loop_breed.name == "skaven_slave") and (not bot_settings.assigned_enemies[loop_unit] or bot_settings.assigned_enemies[loop_unit] == self_unit or #prox_enemies < bot_count or loop_is_after_me)) or (loop_breed.name == "skaven_loot_rat" and loop_dist < bot_settings.skaven_loot_rat.melee_range.default) then
				...
				-- if loop_is_after_me and d.defense.awareness.attacks[loop_unit] and (d.current_time - d.defense.awareness.attacks[loop_unit] > aggressive_melee_allowance) and loop_dist < closest_trash_threat.distance then
				-- if loop_is_after_me and loop_blackboard.attack_anim and loop_dist < closest_trash_threat.distance then
			...
			elseif loop_breed.name == "skaven_storm_vermin_commander" or loop_breed.name == "skaven_storm_vermin" then
				...
				--edit--
				-- Vernon: if this rat has started attack the bot
				-- if loop_is_after_me and d.defense.awareness.attacks[loop_unit] and loop_dist < closest_sv_threat.distance then
				-- if loop_is_after_me and loop_dist < closest_sv_threat.distance then
	...
	-- elseif not sniper_target_selection and closest_trash_threat.unit then
	elseif closest_trash_threat.unit and closest_trash_threat.distance < 3 then
...
--edit--
local last_calculate_dodge_time = {}
--edit--
...
--]]
	...
	local num_trash_try_to_hit = 1
	
	if d.defense.must_push_gutter and d.defense.current_stamina >= 1 then
		d.token.push = true
	end
	...
	-- an attacking SV is in push range and the bot has means to interrupt it with pushing >> push
	if d.defense.can_push_stormvermin then
		d.token.push = true
	-- an SV is attacking this bot >> defend & (try to) dodge
	elseif #d.defense.threat_stormvermin > 0 then
		d.token.block = true
		-- d.token.dodge = true
		-- if d.defense.dodge_count_left and d.defense.dodge_count_left >= -1 and not d.defense.is_dodging then
		if d.defense.dodge_count_left and d.defense.dodge_count_left >= -1 then
			d.token.dodge = true
		-- else
			-- Vernon: let's see how risky this is...
			-- d.token.block = true
		end
	end
	
	if #d.defense.threat_trash > num_trash_try_to_hit and d.defense.current_stamina >= 1 then
		d.token.push = true
	end
	...
	if not last_calculate_dodge_time[unit] or 0.125 < d.current_time - last_calculate_dodge_time[unit] then
		if #d.defense.threat_trash > 0 and #d.defense.threat_trash <= num_trash_try_to_hit
			and not (d.defense.threat_running_attack and d.defense.is_dodging)
			-- and not d.defense.threat_running_attack
			and d.defense.dodge_count_left and d.defense.dodge_count_left >= 0
			and d.defense.current_stamina >= 4
			and #d.enemies.melee < 5	--4
			and not d.token.push and not d.token.block then
			--
			-- d.token.dodge = true
			calculate_dodge = true
		-- Vernon: try to dodge when out of stamina
		elseif #d.defense.threat_trash == 1 and d.defense.current_stamina < 1
			and not d.defense.is_dodging and d.defense.dodge_count_left and d.defense.dodge_count_left >= 1 and #d.enemies.melee < 5 then	--4
			--
			-- d.token.dodge = true
			calculate_dodge = true
		end

		if calculate_dodge and not d.token.dodge and not d.defense.is_dodging then
			last_calculate_dodge_time[unit] = d.current_time
			-- Vernon: codes modified from CharacterStateHelper.check_to_start_dodge
			local locomotion_extension = ScriptUnit.extension(self_unit, "locomotion_system")
			if locomotion_extension and not locomotion_extension.is_falling then
				local dodge_direction = Vector3(0, 0, 0)
				local dodge_input = CharacterStateHelper.get_movement_input(input_ext)
				if dodge_input.y then
					dodge_input.y = math.min(dodge_input.y, 0)
				end
				local input_length = Vector3.length(dodge_input)
				if input_ext.minimum_dodge_input < input_length then
					local normalized_input = dodge_input / input_length
					local x = normalized_input.x
					local y = normalized_input.y
					local abs_x = math.abs(x)
					local forward_ok = y <= 0 or abs_x > 0.707
				...
				--
				local dodge_goal = d.position + dodge_direction * 1.8	-- Vernon: min dodge distance is 1.7 for shields and 2h hammers etc.
				...
				if success then
					dodge_goal.z = z
					local dodge_ok = GwNavQueries.raycango(nav_world, d.position, dodge_goal, traverse_logic)
					if dodge_ok then
						d.token.dodge = true
						start_dodge = true
					end
	...
	if last_calculate_dodge_time[unit] and 5 < d.current_time - last_calculate_dodge_time[unit] then
		last_calculate_dodge_time[unit] = nil
	end
	
	-- there are trash rats around that threat the bot >> defend
	-- if #d.defense.threat_trash > 0 and (d.defense.current_stamina >= 1 or has_second_wind) then
	if not start_dodge and #d.defense.threat_trash > 0 and (d.defense.current_stamina >= 1 or has_second_wind) then
		d.token.block = true
	end
...
--edit--
	...
	--edit--
	-- dodging should not stop attacking
	-- if d.token.block or d.token.push or d.token.dodge then
...
Mods.hook.set(mod_name, "CharacterStateHelper.check_to_start_dodge", function(func, unit, input_extension, status_extension, t)
	...
	if is_bot then
		...
		--edit--
		if need_dodge_direction_fix[unit] and input_extension.move.y then
			input_extension.move.y = math.min(input_extension.move.y, 0)
			need_dodge_direction_fix[unit] = false
		end
		--edit--
	end
	...
	elseif d.settings.better_melee then
		if d.equipped.melee then
			...
			if d.token.dodge then
				...
				--edit--
				need_dodge_direction_fix[unit] = true
				--edit--
			end
	...
	if (follow_unit and may_follow_player) or (follow_unit_table and table.has_item(follow_unit_table, player_unit)) then
		...
		for _, human_player in pairs(human_players) do
			...
			if loop_active then
				...
				if loop_distance_squared and 1 < loop_distance_squared and loop_distance_squared < 3 then
		...
		repositioning_denied = repositioning_denied or too_close_to_human
			...
			for unit, data in pairs(bot_ai_data) do
				...
				if loop_pos then
					...
					if old_position then
						...
						if not repositioning_denied and Vector3.length(old_position - best_point) > 1 then
