...
Mods.hook.set(mod_name, "BTBotMeleeAction._update_melee", function(func, self, unit, blackboard, dt, t)
	...
	if (in_melee_range and not defending and d.melee_attack_allowed) and
		...
		brekable_target or not d.regroup.enabled or
			(d.regroup.enabled and target_in_front and target_breed ~= "skaven_rat_ogre")) and not DEBUG_NO_MELEE_ATTACKS then
...
Mods.hook.set(mod_name, "PlayerBotBase._update_movement_target", function(func, self, dt, t)
	...
	local override_melee         = blackboard.melee and blackboard.melee.engage_position_set and
		blackboard.navigation_destination_override:unbox()
	local override_rat_ogre_flee = blackboard.navigation_flee_destination_override and
		blackboard.navigation_flee_destination_override:unbox()
	local cover_position         = tostring(blackboard.taking_cover.cover_position:unbox()) ~=
		"Vector3(1.#INF, 1.#INF, 1.#INF)"
	...
	if blackboard.target_ally_need_type == "knocked_down" then
		...
		if can_interact_with_ally and
			...
			destination_reached or ally_distance < 1.75 or
				Vector3.length_squared(blackboard.locomotion_extension:current_velocity()) <
				bot_stuck_threshold * bot_stuck_threshold) then
			return true
		end
	end
	...
	if 0 < bot_delta then
		...
		while bots_spawned < bot_delta do
			...
			if owner_type == "available" then
				...
				-- EchoConsole('\n')
				-- EchoConsole("profile_index: " .. profile_index)
				-- EchoConsole("SPProfiles[profile_index].display_name: " .. SPProfiles[profile_index].display_name)
				-- EchoConsole("local_player_id: " .. local_player_id)
				-- EchoConsole("local_peer_id: " .. local_peer_id)
				-- EchoConsole('\n')
		...
		for _, bot_player in ipairs(self._spawn_list) do
			...
			if player_manager.player(player_manager, bot_peer_id, bot_local_player_id) == bot_player then
				...
				if status.health_state ~= "dead" and status.health_state ~= "respawn" and status.health_state ~= "respawning" then
					...
					-- EchoConsole("rotation: " .. tostring(rotation))
					-- EchoConsole("ammo: " .. tostring(ammo))
					-- EchoConsole(Mods.debug:table_to_string(ammo, 3, "ammo"))
