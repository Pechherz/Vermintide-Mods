...
--[[
Version 20220116 - Bug fixes
* Fixed crash regarding missing unit_alive function in newly implemented hook.
* Fixed crash regarding "Hand placed bot nav transition" that could happen on loading into some maps.

Version 20220115 - Release 3 update 3
* Fixed a situation where a bot would try to heal itself with a tome.
* Adjusted healshare carrier auto-heal behavior. A healshare bot carrying medkit should now be less prone to use the medkit on ally if more hp would be gained by utilizing the healshare.
* Bots should now more reliably assist allies hanging from ledges as long as they are close enough to do so.
* Some tweaking to bot navigation.
* Removed revision history text prior to release 1 in order to fit the code to 512kB limit of pastebin.
			...
			elseif data_key == "ammo_percent" then
				...
				if ammo_data then
					...
					local ammo_extension = (
						right_unit and ScriptUnit.has_extension(right_unit, "ammo_system") and
							ScriptUnit.extension(right_unit, "ammo_system")) or
						(
						left_unit and ScriptUnit.has_extension(left_unit, "ammo_system") and ScriptUnit.extension(left_unit, "ammo_system")
						)
...
local get_mission_defend_position = function(follow_target_position)
	...
	local stand_v1 = Vector3(math.sin(base_angle_offset_rad * 0 + angle_offset_rad),
		math.cos(base_angle_offset_rad * 0 + angle_offset_rad), 0)
	local stand_v2 = Vector3(math.sin(base_angle_offset_rad * 1 + angle_offset_rad),
		math.cos(base_angle_offset_rad * 1 + angle_offset_rad), 0)
	local stand_v3 = Vector3(math.sin(base_angle_offset_rad * 2 + angle_offset_rad),
		math.cos(base_angle_offset_rad * 2 + angle_offset_rad), 0)
	--
	...
	if unit_pos and follow_unit_pos then
		...
		elseif level_id == "dlc_stromdorf_town" then
			...
			elseif (Vector3.length(follow_unit_pos - Vector3(74.95, 22.42, 10.80)) < 4) and
				(
				Vector3.length(unit_pos - Vector3(85.00, 24.49, 10.50)) < 4 or
					Vector3.length(unit_pos - Vector3(88.97, 16.57, 10.47)) < 4) then
				ret = Vector3(74.95, 22.42, 10.80)
			end
		elseif level_id == "docks_short_level" then
			if (Vector3.length(follow_unit_pos - Vector3(8.73, 6.41, 0.34)) < 4 and (follow_unit_pos.z - 0.34) > -0.2) and
				(
				Vector3.length(unit_pos - Vector3(8.39, -0.17, -2.97)) < 3 or
					Vector3.length(unit_pos - Vector3(8.98, -2.46, -3.23)) < 3 or
					Vector3.length(unit_pos - Vector3(10.65, 4.45, -3.33)) < 3) then
				ret = Vector3(3.37, 7.08, 0.18)
			elseif (Vector3.length(follow_unit_pos - Vector3(8.93, 1.66, -3.37)) < 4 and (follow_unit_pos.z - -3.37) > -0.2) and
				(Vector3.length(unit_pos - Vector3(8.73, 6.41, 0.34)) < 10 and math.abs(unit_pos.z - 0.34) < 1) then
				ret = Vector3(2.73, 0.00, -2.70)
			elseif (Vector3.length(follow_unit_pos - Vector3(53.32, 5.46, 0.84)) < 5) and
				(
				(Vector3.length(unit_pos - Vector3(48.54, 0.65, 4.95)) < 7 and math.abs(unit_pos.z - 4.95) < 1) or
					Vector3.length(unit_pos - Vector3(37.53, -2.46, 0.68)) < 4) then
				ret = follow_unit_pos
			end
			--
	...
	if Unit.alive(unit) and status_extension and not status_extension:is_disabled() then
		...
		if ammo_data then
			...
			if inventory_extension.current_overcharge_status then
				-- fire weapon / overcharge status for local player / bots
				local current_oc, threshold_oc, max_oc = inventory_extension.current_overcharge_status(inventory_extension,
					"slot_ranged")
				-- Added checks to return clause to prevent chrashes with anyweapon mod. Thanks to VernonKun for reporting the issue and providing sample solution.
...
local is_positioned_between_self_and_ally = function(self_unit, ally_unit, enemy_unit)
	if self_unit and ally_unit and enemy_unit then
		...
		if self_pos and ally_pos and enemy_pos then
			...
			if self_to_enemy_dist < ally_dist and ally_to_enemy_dist < ally_dist and
				(self_to_enemy_dist + ally_to_enemy_dist) < zone_dist then
...
local check_line_of_sight = function(self_unit, target_unit, pos1, pos2)
	...
	if (self_unit and target_unit) or (pos1 and pos2) then
		...
		if self_pos_raw and target_pos_raw then
			...
			local hit, hit_pos = physics_world:immediate_raycast(self_pos, dir, dist, "closest", "collision_filter",
...
local is_secondary_within_cone = function(self_unit, target_unit, secondary_unit, cone_angle_deg)
	...
	if Unit.alive(self_unit) and Unit.alive(target_unit) and Unit.alive(secondary_unit) then
		...
		if Vector3.length(target_vector) > 0 and Vector3.length(secondary_vector) > 0 then
			...
			local secondary_deviation_angle_cos = Vector3.dot(Vector3.normalize(target_vector),
				Vector3.normalize(secondary_vector))
					...
					if not loop_disabled then -- if the human is disabled then he/she isn't going to be pickuing up anything
						...
						if not player_near_with_empty_slot and loop_pickup_distance < nearby_distance then -- check if a human player is somewhere near
							if pickup_slot then -- ok, human player is near, does the player have something in the appropriate slot?
								local loop_inventory_item_data = loop_inventory_extension.get_slot_data(loop_inventory_extension, pickup_slot) or
									false
...
local is_weapon_charged = function(self_unit)
	...
	local wielded_slot_name = (inventory_extension and inventory_extension.get_wielded_slot_name(inventory_extension)) or
		false
					...
					for _, chain_info in pairs(action.allowed_chain_actions) do
						if chain_info and chain_info.sub_action then
							...
							if ranged_equipped then
								...
								if spread_ok and after_start and before_end and
									(chain_info.input == "action_one" or (not self_owner.bot_player and chain_info.input == "action_one_mouse")) and
									ranged_keyword_match then
	...
	local weapon_charged = (melee_equipped and melee_weapon_charged) or
		(ranged_equipped and ranged_weapon_charged and (not trueflight_aim_active or trueflight_aim_target ~= nil))
...
local reload_bot_ranged_weapon = function(d)
	...
	local ally_needs_pickup = blackboard.target_ally_need_type == "knocked_down" or
		blackboard.target_ally_need_type == "ledge" or blackboard.target_ally_need_type == "hook"
...
local bot_settings =
{
	...
	menu_settings =
	{
		...
		update = function(self)
			...
			self.heal_only_if_wounded_healshare  = (self.heal_only_if_wounded == 1 or self.heal_only_if_wounded == 2) and
				not deathwish_enabled
			...
			self.pass_draughts_to_humans         = PASS_DRAUGHTS_TO_HUMANS_DECODE[(get(me.SETTINGS.PASS_DRAUGHTS_TO_HUMANS) or 4)
				] or "Default"
	...
	scaler_data =
	{
		...
		is_allowed_to_release_from_disabler = function(self, disabler_unit)
			if disabler_unit and Unit.alive(disabler_unit) and self.specials_release_denied[disabler_unit] then
				...
				local release_allowed                 = current_time >= self.specials_release_denied[disabler_unit] or
					enemy_has_been_pinged or enemy_is_pinged or enemy_has_been_damaged_by_human
		...
		add_offense_tokens = function(self, amount)
			...
			local ranged_threshold_tokens = 0.8 * (self.offense_ranged_scaling_stop - self.offense_ranged_scaling_start) +
				self.offense_ranged_scaling_start
		...
		get_specials_release_delay = function(self, ally_unit)
			...
			local clamp_min            = specials_balance_min +
				((specials_balance_max - specials_balance_min) * (1 - cap_specials))
...
local d_data_update = function(d)
	...
	local local_player_unit = (
		Managers.player and Managers.player:local_player() and Managers.player:local_player().player_unit) or d.unit
	...
	d.follow_target               = (d.settings.always_follow_host and not local_player_disabled and local_player_unit) or
		blackboard.ai_bot_group_extension.data.follow_unit
	...
	if not d.extensions_ready then
		...
		d.extensions_ready     = (
			d.extensions.status and d.extensions.inventory and d.extensions.buff and d.extensions.input and true) or false
	end
	...
	local equipped_slot                             = (
		d.extensions.inventory and d.extensions.inventory:get_wielded_slot_name()) or false
	...
	elseif d.equipped.ranged then
		...
		d.equipped.weapon_template         = weapon_template
		d.equipped.weapon_extension        = ScriptUnit.has_extension(weapon_unit, "weapon_system")
		d.equipped.ranged_can_reload       = (
			d.equipped.weapon_extension.ammo_extension and d.equipped.weapon_extension.ammo_extension:can_reload()) or false
	...
	if not d.extensions.inventory:has_ammo_consuming_weapon_equipped() then
		if not d.extensions.overcharge then
			...
			local right_hand_overcharge_extension = ScriptUnit.has_extension(slot_data.right_unit_1p, "overcharge_system") and
				ScriptUnit.extension(slot_data.right_unit_1p, "overcharge_system")
			local left_hand_overcharge_extension  = ScriptUnit.has_extension(slot_data.left_unit_1p, "overcharge_system") and
				ScriptUnit.extension(slot_data.left_unit_1p, "overcharge_system")
		...
		if d.extensions.overcharge then
			...
			if d.extensions.buff then
				...
				if has_channeling_rune then
					predicted_vent_damage_tick = d.extensions.buff:apply_buffs_to_value(predicted_vent_damage_tick,
						StatBuffIndex.REDUCED_VENT_DAMAGE)
				end
			end
		end
	end
	...
	d.enough_hp_to_vent   = d.vent_allowed and
		(
		(d.current_hitpoint_percentage > d.vent_hp_threshold and not venting_threats_life and not wounded) or
			d.equipped.heat_amount < d.equipped.heat_threshold)
	...
	local filter_unit_list_by_distance = function(distance, unit_list)
		...
		for _, loop_unit in pairs(unit_list) do
			...
			local valid_target  = not validity_check or
				not (loop_breed.name == "skaven_clan_rat" or loop_breed.name == "skaven_slave") or
				(loop_bb.target_unit or loop_bb.attacking_target or loop_bb.special_attacking_target)
	...
	local push_radius       = (
		bot_settings.lookup.push_radius[d.equipped.weapon_id] or bot_settings.lookup.push_radius.default) + 0.5
	...
	for _, loop_unit in pairs(d.enemies.push) do
		...
		elseif loop_breed.name == "skaven_gutter_runner" then
			...
			if loop_bb.target_dist < 0.1 then
				...
				if may_release_ally and not bot_settings.lookup.can_interrupt_gutter[d.equipped.weapon_id] and
					d.defense.current_stamina >= 1 then
	...
	for _, loop_unit in pairs(d.enemies.medium) do
		...
		elseif loop_breed.name == "skaven_storm_vermin_commander" or loop_breed.name == "skaven_storm_vermin" then
			...
			if loop_bb.attacking_target or loop_bb.special_attacking_target then -- attacking_target for push, special_attacking_target for other attacks
				...
				local enemy_attack_duration = (
					d.settings.aggressive_melee and (d.current_time - d.defense.awareness.attacks[loop_unit])) or math.huge
				...
				if enemy_attack_duration > aggressive_melee_allowance and
					(loop_in_front or (loop_distance < 3 and not loop_flanking)) then
					...
					if d.equipped.melee then
						...
						local has_heavy_push = (
							d.equipped.weapon_template.defense_meta_data and d.equipped.weapon_template.defense_meta_data.push == "heavy") or
							false
	...
	for _, loop_unit in pairs(d.enemies.bosses) do
		...
		if loop_breed.name == "skaven_storm_vermin_champion" then
			...
			if loop_bb.attacking_target or loop_bb.special_attacking_target then
				...
				local distance_to_krench_target = (
					krench_target and d.unit and POSITION_LOOKUP[krench_target] and POSITION_LOOKUP[d.unit] and
						Vector3.distance(POSITION_LOOKUP[krench_target], POSITION_LOOKUP[d.unit])) or 0
				local spin_aoe                  = attack_anim == "attack_spin" or attack_anim == "attack_spin_charge"
				local line_aoe                  = attack_anim == "charge_attack_step" or attack_anim == "charge_attack_lunge" or
					attack_anim == "attack_charge_shatter"
				local jump_strike               = attack_anim == "attack_run_2"
				local regular_attack            = attack_anim == "attack_special" or attack_anim == "attack_sweep_left" or
	...
	local target_is_stormvermin = target_breed and
		(target_breed.name == "skaven_storm_vermin" or target_breed.name == "skaven_storm_vermin_commander")
	...
	d.keep_last_stormvermin_alive = able_human and last_stand_map and target_is_stormvermin and #alive_sv < 2 and
		#alive_trash < 20 and #alive_bosses < 1
...
local get_d_data = function(self_unit)
	...
	if not d then
		...
		if blackboard.target_unit then
			...
			d_tmp.melee.may_engage        = Vector3.length(d_tmp.regroup.position - d_tmp.melee.target_position) <
				(d_tmp.regroup.distance_threshold - 0.1)
...
local may_pick_healslot_item = function(self_unit, item_unit, may_swap_heals, may_drop_tome, may_pick_tome,
                                        forced_to_loot)
	...
	local other_need_swap      = (other_need_draught and other_extra_draught and not item_is_draught) or
		(other_need_medkit and other_extra_medkit and not item_is_medkit)
	...
	local has_what_other_need  = (db.extra_draught and other_need_draught) or
		...
		(
		not item_is_draught and db.extra_draught and not (db.heavy_hurt or db.wounded) and
			(other_heavy_hurt_no_heal or other_wounded_no_heal))
	...
	if may_pick_grenadeslot_item_frame_checker:is_new_frame() then
		...
		for _, loop_owner in pairs(bots) do
			...
			if extensions_ok and not status_extension:is_disabled() then
				...
				local can_pick_grenade     = empty_slot or (item_is_frag_grenade and not frag_grenade) or
					(item_is_fire_grenade and not fire_grenade)
...
local find_best_healer = function(target_unit, ignore_human_healers)
	...
	for _, healer_unit in pairs(player_list) do
		...
		local wounded          = (
			status_extension and not status_extension:has_wounds_remaining() and not deathwish_enabled and true) or false
		...
		local is_healsharing    = blackboard and blackboard.is_healing_self == true and healshare and
			target_distance < HEALSHARE_RANGE
	...
	local best_draught_potential = (
		(not ignore_human_healers or target_is_bot) and player_data[target_unit].draught and
			not player_data[target_unit].healshare and
			player_data[target_unit].draught_potential * (player_data[target_unit].draught_potential / 75)) or 0
	--
		...
		if is_bot or not ignore_human_healers then
			...
			if data.can_healshare_target and (team_wounded or not healshare_needs_wounded) then
				...
				if data.draught then
					...
					healshare_effectiveness = healshare_effectiveness +
						((data.draught_potential - data.healshare_potential) / data.hp_max)
				elseif data.medkit then
					healshare_potential     = healshare_potential - data.healshare_potential + data.medkit_potential
					healshare_effectiveness = healshare_effectiveness +
						((data.medkit_potential - data.healshare_potential) / data.hp_max)
				end
			...
			if data.dove_ready and may_use_medkit then
				...
				local dove_effectiveness           = data.medkit_potential / data.hp_max +
					player_data[target_unit].medkit_potential / player_data[target_unit].hp_max
...
Mods.hook.set(mod_name, "ActionBeam.client_owner_start_action", function(func, self, new_action, t)
	...
	local is_third_person_mod = Mods.ThirdPerson and Mods.ThirdPerson.SETTINGS and Mods.ThirdPerson.SETTINGS.ACTIVE and
		Mods.ThirdPerson.SETTINGS.ACTIVE.save and Application.user_setting(Mods.ThirdPerson.SETTINGS.ACTIVE.save)
...
--]]
SpawnManager._update_available_profiles = function(self, profile_synchronizer, available_profile_order,
                                                   available_profiles)
	local delta = 0
	local bots = 0
	local humans = 0
	...
	for _, profile_index in ipairs(profile_indexes) do
		...
		elseif (owner_type == "available" or owner_type == "bot") and
			not table.contains(available_profile_order, profile_index) then
	...
	if self._forced_bot_profile_index then
		...
		local available = (
			index and profile_synchronizer.owner_type(profile_synchronizer, forced_bot_profile_index) == "available") or false
...
Mods.hook.set(mod_name, "PlayerBotBase._update_target_enemy", function(func, self, dt, t)
	...
	local offense_token_passive_cap = bot_settings.scaler_data.offense_token_passive_cap *
		bot_settings.scaler_data:get_difficulty_modifier_life()
	local offense_scaler_melee_multiplier = math.clamp((
		(bot_settings.scaler_data.offense_tokens / 2) / offense_token_passive_cap), 0, cap_melee)
	local offense_scaler_ranged_multiplier = 0
	if new_frame then
		update_target_enemy_frame_checker:update()
		local offense_ranged_scaling_start = bot_settings.scaler_data.offense_ranged_scaling_start *
			bot_settings.scaler_data:get_difficulty_modifier_life()
		local offense_ranged_scaling_stop = bot_settings.scaler_data.offense_ranged_scaling_stop *
			bot_settings.scaler_data:get_difficulty_modifier_life()
		offense_scaler_ranged_multiplier = math.clamp((
			(bot_settings.scaler_data.offense_tokens - offense_ranged_scaling_start) /
				(offense_ranged_scaling_stop - offense_ranged_scaling_start)), 0, cap_ranged)
		bot_settings.scaler_data.offense_ranged_range = offense_scaler_ranged_multiplier *
			bot_settings.trash_SV_detection_distance
		d.ranged_attack_allowed = d.current_time >=
			d.last_ranged_kill + (bot_settings.scaler_data.offense_max_delay_ranged * (1 - offense_scaler_ranged_multiplier))

		if t >= bot_settings.scaler_data.offense_time_pool_timer then
			bot_settings.scaler_data.offense_time_pool_timer = math.max(bot_settings.scaler_data.offense_time_pool_timer + 1,
				t -
				5)
			bot_settings.scaler_data.offense_time_pool = bot_settings.scaler_data.offense_time_pool +
				offense_scaler_melee_multiplier
		...
		if t > bot_settings.scaler_data.damage_taken_debt_trasfer_next_tick then
			while bot_settings.scaler_data.damage_taken_debt_trasfer_next_tick <= t do
				bot_settings.scaler_data.damage_taken_debt_trasfer_next_tick = bot_settings.scaler_data.damage_taken_debt_trasfer_next_tick
					+ 1
			end
			local factor                                      = bot_settings.scaler_data.damage_taken_debt_trasfer_factor
			local tick_min                                    = math.min(bot_settings.scaler_data.damage_taken_debt_trasfer_min,
				math.max(bot_settings.scaler_data.damage_taken_debt_buffer, 0))
			local tick_max                                    = bot_settings.scaler_data.damage_taken_debt_trasfer_max
			local tick_value                                  = math.clamp(bot_settings.scaler_data.damage_taken_debt_buffer *
				factor, tick_min, tick_max)
	...
	if ENABLE_DEBUG_FEEDS and get_hotkey_state("0") then
		...
		if not bot_settings.scaler_data.debug_key_0 then
			EchoConsole("-------------------------------")
			EchoConsole("Off. tokens / cap: " ..
				number_to_string(bot_settings.scaler_data.offense_tokens / offense_token_passive_cap, 2))
	...
	for key, data in pairs(bot_settings.breakable_objectives) do
		if Unit.alive(data.unit) and data.health_extension:is_alive() and
			data.health_extension:current_health_percent() >= bot_settings.menu_settings.breakables_objectives_threshold and
			bot_settings.menu_settings.help_with_breakables_objectives then
	...
	for _, loop_unit in pairs(alive_specials) do
		...
		if loop_los then
			...
			if loop_dist > balance_settings.vision_distance or
				(
				balance_settings.must_have_dealt_damage and not bot_settings.scaler_data.specials_lookup.has_dealt_damage[loop_unit]
				) then
				perception_scaler = 0
			end
	...
	for _, loop_unit in pairs(prox_enemies) do
		...
		local loop_is_after_me = loop_blackboard and
			(
			loop_blackboard.target_unit == self_unit or loop_blackboard.attacking_target == self_unit or
				loop_blackboard.special_attacking_target == self_unit)
		...
		if loop_dist and loop_blackboard and loop_blackboard.target_unit then
			if (
				(loop_breed.name == "skaven_clan_rat" or loop_breed.name == "skaven_slave") and
					(
					not bot_settings.assigned_enemies[loop_unit] or bot_settings.assigned_enemies[loop_unit] == self_unit or
						#prox_enemies < bot_count or loop_is_after_me)) or
				(loop_breed.name == "skaven_loot_rat" and loop_dist < bot_settings.skaven_loot_rat.melee_range.default) then
				-- ** trash rats **
		...
		for _, separation_check_unit in pairs(separation_check_units) do
			if loop_breed.name ~= "skaven_rat_ogre" and loop_breed.name ~= "skaven_storm_vermin_champion" and
				loop_dist_self < separation_threat.distance then
	...
	for _, loop_unit in pairs(detected_specials) do
		...
		if loop_los and loop_breed then
			...
			local loop_dist = self_pos and loop_pos and Vector3.distance(self_pos, loop_pos)
			if loop_dist then
				local immediate_threat_range  = (
					bot_settings[loop_breed.name] and bot_settings[loop_breed.name].immediate_threat_range) or
					bot_settings.default.immediate_threat_range
				local blocking_melee_distance = (
					bot_settings[loop_breed.name] and bot_settings[loop_breed.name].melee_range and
						bot_settings[loop_breed.name].melee_range.reduced) or 0
				if (loop_dist < closest_special.distance and closest_trash.distance > blocking_melee_distance) or
					...
					--
					local immediate_threat_range_pinged = (
						bot_settings[loop_breed.name] and bot_settings[loop_breed.name].immediate_threat_range_pinged) or
	...
	elseif closest_boss.unit and -- there must be a boss to target it
		...
		#prox_enemies == 1 or -- only 1 enemy around (the boss)
			...
			(closest_boss.distance < 5 and not closest_boss.flanking) or
			-- this bot is in front of the boss and too close (target it to enable bot flanking behavior)
	...
	if blackboard.target_unit then
		...
		d.melee.target_distance   = (
			self_melee_position and d.melee.aim_position and Vector3.length(self_melee_position - d.melee.aim_position)) or
			Vector3.length(d.melee.target_offset)
		d.melee.is_aiming         = d.regroup.distance < d.regroup.distance_threshold + 1
		d.melee.may_engage        = Vector3.length(d.regroup.position - d.melee.target_position) <
			(d.regroup.distance_threshold - 0.1)
...
Mods.hook.set(mod_name, "BTBotMeleeAction._attack", function(func, self, unit, blackboard, input_ext, target_unit)
	...
	local must_use_light_attack_against_armor = penetrating_light_attack or
		(killing_blow_light_attack and kb_against_armor
		)
...
Mods.hook.set(mod_name, "BTBotMeleeAction._defend",
	function(func, self, unit, blackboard, target_unit, input_ext, t, in_melee_range)
		local self_unit  = unit
		local self_owner = Managers.player:unit_owner(self_unit)
		local d          = get_d_data(self_unit)
		...
		if d.token.block or d.token.push or d.token.dodge then
			...
			while (bot_settings.scaler_data.damage_taken_roll_count > 0) do
				local rng_check = math.clamp(bot_settings.scaler_data.damage_taken_debt /
					(bot_settings.scaler_data:get_difficulty_modifier_damage() * 37.5 * 15), 0, 1)
				if not d.token.take_damage then
					if bot_settings.scaler_data.damage_taken_token_available > 0 and d.current_time > d.take_damage_cooldown and
						math.random() < rng_check then
						bot_settings.scaler_data.damage_taken_token_available = bot_settings.scaler_data.damage_taken_token_available - 1
...
Mods.hook.set(mod_name, "BTBotMeleeAction._update_melee", function(func, self, unit, blackboard, dt, t)
	...
	local normalized_damage_taken_debt = bot_settings.scaler_data.damage_taken_debt /
		(37.5 * bot_settings.scaler_data:get_difficulty_modifier_damage())
	...
	local should_engage      = may_engage and
		(
		(not is_unit_in_aoe_explosion_threat_area(target_unit) and not is_unit_in_aoe_explosion_threat_area(self_unit)) or
			is_unit_in_aoe_explosion_threat_area(follow_unit))
	local in_melee_range     = (brekable_target and target_distance <= MAX_MELEE_RANGE) or target_distance < melee_range
	local engage_range_max   = math.min((
		bot_settings.melee.engage_distance[target_breed] or bot_settings.melee.engage_distance.default),
		d.regroup.distance_threshold)
	...
	if (in_melee_range and not defending and d.melee_attack_allowed) and
		(
		brekable_target or not d.regroup.enabled or
			(d.regroup.enabled and target_in_front and target_breed.name ~= "skaven_rat_ogre")) and not DEBUG_NO_MELEE_ATTACKS then
	...
	if in_melee_range and should_engage then
		wants_engage = blackboard.aggressive_mode or (already_engaged and t - melee_bb.engage_change_time < 5) or
			(target_breed and target_breed.name == "skaven_rat_ogre")
		wants_engage = true
	...
	local engage = wants_engage and
		self:_allow_engage(self_unit, target_unit, blackboard, action_data, already_engaged, self_aim_pos, d.regroup.position)
...
Mods.hook.set(mod_name, "CharacterStateHelper.check_to_start_dodge",
	function(func, unit, input_extension, status_extension, t)
...
Mods.hook.set(mod_name, "BTBotShootAction._may_attack",
	function(func, self, enemy_unit, shoot_blackboard, range_squared, t)
		...
		local sufficiently_charged = not shoot_blackboard.minimum_charge_time or
			(not shoot_blackboard.always_charge_before_firing and not charging) or (charging and is_weapon_charged(self_unit))
		local max_range_squared = (charging and shoot_blackboard.max_range_squared_charged) or
			shoot_blackboard.max_range_squared

		local breed         = Unit.get_data(enemy_unit, "breed")
		local scaled_target = breed.name == "skaven_slave" or breed.name == "skaven_clan_rat" or
			breed.name == "skaven_loot_rat" or breed.name == "skaven_storm_vermin" or
			breed.name == "skaven_storm_vermin_commander"

		local may_fire = (d.ranged_attack_allowed or not scaled_target) and sufficiently_charged and
			not shoot_blackboard.obstructed and range_squared < max_range_squared
...
Mods.hook.set(mod_name, "BTConditions.bot_in_melee_range", function(func, blackboard)
	...
	local target_is_trash = breed.name == "skaven_slave" or breed.name == "skaven_clan_rat" or
		breed.name == "skaven_loot_rat"
	local target_is_SV    = breed.name == "skaven_storm_vermin" or breed.name == "skaven_storm_vermin_commander"
	local scaler_in_range = breed.special or breed.boss or
		(distance_to_target <= bot_settings.scaler_data.offense_ranged_range)
	...
	if bot_settings[breed.name] == nil or bot_settings[breed.name].melee_range == nil then
		...
		local threshold_factor = (d.enough_hp_to_vent and bot_settings.heat_threshold.vent_threshold_factor) or
			bot_settings.heat_threshold.passive_threshold_factor
...
Mods.hook.set(mod_name, "BTConditions.has_priority_or_opportunity_target", function(func, blackboard)
	...
	local result            = (
		target == blackboard.opportunity_target_enemy or target == blackboard.priority_target_enemy or
			target == blackboard.urgent_target_enemy) and distance_ok and has_line_of_sight
...
Mods.hook.set(mod_name, "PlayerBotInput.update", function(func, self, unit, input, dt, context, t)
	...
	if d.settings.better_melee and d.equipped.melee and d.melee.target_unit then
		...
		if d.token.melee_light then
			...
			local current_time_in_action = (current_action_settings and (t - d.equipped.weapon_extension.action_time_started)) or
				0
			...
			local target_alive = is_unit_alive(d.melee.target_unit)
			local preferred_chain_length = bot_settings.lookup.light_melee_chain_length[d.equipped.weapon_id] or
				bot_settings.lookup.light_melee_chain_length.default
			...
			if not target_alive then
				...
				self._input.action_one_release = true
				self._defend = true
			elseif not current_action_settings and d.equipped.melee_chain_state ~= "cancel_start" and
				d.equipped.melee_chain_state ~= "cancel_blocking" then
			...
			elseif d.equipped.melee_chain_state == "cancel_start" or d.equipped.melee_chain_state == "cancel_blocking" or
				d.equipped.melee_chain_state == "cancel_done" or
				(after_start and not before_end and d.equipped.melee_chain_position >= preferred_chain_length) then
				if d.equipped.melee_chain_state ~= "cancel_start" and d.equipped.melee_chain_state ~= "cancel_blocking" and
					d.equipped.melee_chain_state ~= "cancel_done" then
...
Mods.hook.set(mod_name, "PlayerWhereaboutsExtension._check_bot_nav_transition",
	function(func, self, nav_world, input, current_position)
		if bot_settings.menu_settings.bot_file_enabled then
			...
			if in_valid_position then
				...
				last_valid_position = (nav_fix.last_valid_position[self_unit] and nav_fix.last_valid_position[self_unit]:unbox()) or
					false
			end
...
Mods.hook.set(mod_name, "BotNavTransitionManager.create_transition",
	function(func, self, from, via, wanted_to, player_jumped, make_permanent)
...
Mods.hook.set(mod_name, "BTConditions.has_target_and_ammo_greater_than", function(func, blackboard, args)
	...
	local enemy_ammo_threshold_selector = (BTConditions.has_priority_or_opportunity_target(blackboard) and "special") or
		"default"
...
Mods.hook.set(mod_name, "BTConditions.cant_reach_ally", function(func, blackboard)
	...
	return blackboard.moving_toward_follow_position and
		(((disable_bot_main_path_teleport_check or is_forwards) and 1) or 5) < fails and 3 < t - last_success and
		not blackboard.has_teleported
end)
...
Mods.hook.set(mod_name, "BTBotShootAction._calculate_aim_speed",
	function(func, self, self_unit, dt, current_yaw, current_pitch, wanted_yaw, wanted_pitch, current_yaw_speed,
	         current_pitch_speed)
		local self_owner = Managers.player:unit_owner(self_unit)
		local d          = get_d_data(self_unit)

		if not d or not d.settings.better_ranged or not d.settings.bot_file_enabled then
			return func(self, self_unit, dt, current_yaw, current_pitch, wanted_yaw, wanted_pitch, current_yaw_speed,
				current_pitch_speed)
		end
...
Mods.hook.set(mod_name, "BTBotShootAction._aim_good_enough",
	function(func, self, dt, t, shoot_blackboard, yaw_offset, pitch_offset)
		local self_unit  = shoot_blackboard.unit
		local self_owner = Managers.player:unit_owner(self_unit)
		local d          = get_d_data(self_unit)
		...
		if bb.reevaluate_aim_time < t then
			...
			if (aim_data.max_radius / 50) < offset then
				...
				if offset < aim_data.min_radius then
					...
					local prob = math.auto_lerp(aim_data.min_radius, aim_data.max_radius, aim_data.min_radius_pseudo_random_c,
						aim_data.max_radius_pseudo_random_c, offset) * num_rolls
...
Mods.hook.set(mod_name, "BTBotShootAction._wanted_aim_rotation",
	function(func, self, self_unit, target_unit, current_position, projectile_info, projectile_speed, aim_at_node)
		local self_owner = Managers.player:unit_owner(self_unit)
		local d          = get_d_data(self_unit)
		...
		local target_rotation, target_position = func(self, self_unit, target_unit, current_position, passed_projectile_info,
			projectile_speed, aim_at_node)
		...
		if d.equipped.ranged then
			...
			if weapon_type == "bw_staff_geiser" and projectile_info == nil then -- conflag staff charged attack
				...
				if target_pos then
					...
					local ret_rot = Quaternion.multiply(Quaternion.look(Vector3.normalize(Vector3.flat(ret_pos - current_position)),
						Vector3.up()), Quaternion(Vector3.right(), required_aim_angle))
...
Mods.hook.set(mod_name, "BTBotShootAction._aim", function(func, self, unit, blackboard, dt, t)
	...
	local action_data = self._tree_node.action_data
	local yaw_offset, pitch_offset, wanted_aim_rotation, actual_aim_rotation, actual_aim_position = self:_aim_position(dt, t
		, unit, camera_position, camera_rotation, target_unit, shoot_bb)

	if shoot_bb.reevaluate_obstruction_time < t then
		if self:_reevaluate_obstruction(unit, shoot_bb, action_data, t, World.get_data(blackboard.world, "physics_world"),
			camera_position, wanted_aim_rotation, unit, target_unit, actual_aim_position) then
	...
	if self:_aim_good_enough(dt, t, shoot_bb, yaw_offset, pitch_offset) and
		self:_may_attack(target_unit, shoot_bb, range_squared, t) then
...
Mods.hook.set(mod_name, "BTBotShootAction._is_shot_obstructed",
	function(func, self, physics_world, from, direction, self_unit, target_unit, actual_aim_position, collision_filter)
		local self_owner = Managers.player:unit_owner(self_unit)
		local d          = get_d_data(self_unit)
...
Mods.hook.set(mod_name, "BTBotShootAction._should_charge",
	function(func, self, shoot_blackboard, range_squared, target_unit)
		local self_unit  = shoot_blackboard.unit
		local self_owner = Managers.player:unit_owner(self_unit)
		local d          = get_d_data(self_unit)
		...
		return shoot_blackboard.always_charge_before_firing or shoot_blackboard.charging_shot or
			(shoot_blackboard.charge_range_squared and shoot_blackboard.charge_range_squared < range_squared) or
			(shoot_blackboard.charge_against_armoured_enemy and (not target_breed or target_breed.armor_category == 2))
	end)
...
BTConditions.can_loot_pinged_item = function(blackboard)
	...
	if pinged_tome then
		...
		if tome_posn and tome_posn.x == -73.59222412109375 and tome_posn.y == 250.91647338867187 and
			tome_posn.z == 9.4020509719848633 and (3.5 < Vector3.distance(d.position, tome_posn)) then
			return false
		end
		-- Case : Wizard's tower - 3rd tome - grabbing from below
		if tome_posn and tome_posn.x == 144.01631164550781 and tome_posn.y == 85.939056396484375 and
			tome_posn.z == 60.007415771484375 and (d.position.z < tome_posn.z - 0.5) then
			return false
		end
	...
	if pinged_potion then
		...
		local player_nearby_with_empty_slot, player_within_grabbing_distance = check_players_near_pickup(pinged_object, 20,
			PLAYER_NEAR_PICKUP_DISTANCE)
		...
		if not DEBUG_PARAMETER_20191029 and potion_posn and
			(
			(pickup_distance_reduction and 3.0 < Vector3.distance(d.position, potion_posn)) or
				BOT_ITEM_PICKUP_DISTANCE < Vector3.distance(d.position, potion_posn) or not player_within_grabbing_distance) then
			return false
		end
	...
	if pinged_grenade then
		...
		if not DEBUG_PARAMETER_20191029 and grenade_posn and
			(
			(pickup_distance_reduction and 3.0 < Vector3.distance(d.position, grenade_posn)) or
				BOT_ITEM_PICKUP_DISTANCE < Vector3.distance(d.position, grenade_posn) or not player_within_grabbing_distance) then
			return false
		end
	...
	if pinged_heal then
		...
		if not DEBUG_PARAMETER_20191029 and item_posn and
			(
			(pickup_distance_reduction and 3.0 < Vector3.distance(d.position, item_posn)) or
				BOT_ITEM_PICKUP_DISTANCE < Vector3.distance(d.position, item_posn) or not player_within_grabbing_distance) then
			return false
		end
	...
	if pinged_ammo then
		...
		local player_nearby_uses_ammo, player_within_grabbing_distance = check_players_near_pickup(pinged_object, 20,
			PLAYER_NEAR_PICKUP_DISTANCE)
	...
	if pinged_ammo_crate then
		...
		local _, player_within_grabbing_distance = check_players_near_pickup(pinged_object, 20, PLAYER_NEAR_PICKUP_DISTANCE,
			bot_settings.lookup.pickup_distance_exceptions)
	...
	function(func, self, sender, game_object_id, slot_id, item_name_id, position)
		...
		if Unit.alive(unit) and not ScriptUnit.extension(unit, "status_system"):is_dead() then
			...
			if not owner.remote and owner.bot_player then
				...
				local item_name                                                                                = NetworkLookup.item_names
					[item_name_id]
				local item_is_potion                                                                           = item_name ==
					"potion_damage_boost_01" or item_name == "potion_speed_boost_01"
				local item_is_grenade                                                                          = item_name ==
					"grenade_fire_01" or item_name == "grenade_fire_02" or item_name == "grenade_frag_01" or
		...
		if need_type == "can_accept_heal_item" or need_type == "can_accept_potion" or need_type == "can_accept_grenade" then
			...
			if trading_frame_checker:is_new_frame() then
				...
				for _, self_owner in pairs(Managers.player:bots()) do
					...
					local self_valid            = is_unit_alive(self_unit) and self_status_extension and
						not self_status_extension:is_disabled() -- trading parties are valid only if they exists, are alive, and are not disabled
					local ally_valid            = is_unit_alive(ally_unit) and ally_status_extension and
						not ally_status_extension:is_disabled()
					local ally_need_type        = self_blackboard and self_blackboard.target_ally_need_type
					local valid_ally_need_type  = ally_need_type and
						(
						ally_need_type == "can_accept_heal_item" or ally_need_type == "can_accept_potion" or
							ally_need_type == "can_accept_grenade") -- this should always be true here, but added a check for safety
					...
					if self_valid and ally_valid and not (self_unit == ally_unit) and valid_ally_need_type then
						...
						if distance_to_ally and distance_to_ally < active_trades[ally_unit].distance and may_trade_item and
							not active_trades[ally_unit].trading_started then
...
Mods.hook.set(mod_name, "PlayerBotBase._alter_target_position",
	function(func, self, nav_world, self_position, unit, target_position, reason)
		local network_manager = Managers.state.network
		local unit_id = network_manager.unit_game_object_id(network_manager, self._unit)
		local wasjusttraded = bot_settings.trading.wasjusttraded
		if get(me.SETTINGS.FILE_ENABLED) and
			(reason == "can_accept_grenade" or reason == "can_accept_potion" or reason == "can_accept_heal_item") and
			wasjusttraded[unit_id] then
			return
		else
...
BTConditions.can_ping_storm = function(blackboard)
	...
	if pinged_storm then
		local health_extension = ScriptUnit.has_extension(pinged_storm, "health_system") and
			ScriptUnit.extension(pinged_storm, "health_system")
	...
	for _, enemy_unit in pairs(blackboard.proximite_enemies) do
		if Unit.alive(enemy_unit) and Unit.get_data(enemy_unit, "blackboard").target_unit == self_unit and
			(
			Unit.get_data(enemy_unit, "breed").name == "skaven_storm_vermin_commander" or
				Unit.get_data(enemy_unit, "breed").name == "skaven_storm_vermin") and
			ScriptUnit.extension(enemy_unit, "health_system").current_health_percent(ScriptUnit.extension(enemy_unit,
				"health_system")) > 0 and not pinged_storm then
...
Mods.hook.set(mod_name, "BTConditions.can_revive", function(func, blackboard)
	...
	if blackboard.target_ally_need_type == "knocked_down" then
		...
		if can_interact_with_ally and
			(
			destination_reached or ally_distance < 1.75 or
				Vector3.length_squared(blackboard.locomotion_extension:current_velocity()) < bot_stuck_threshold *
				bot_stuck_threshold) then
			return true
		end
	end
...
Mods.hook.set(mod_name, "BTConditions.bot_should_heal", function(func, blackboard) -- bot healing itself
	...
	local self_unit    = blackboard.unit
	local d            = get_d_data(self_unit)
	local enemy_threat = not d or #d.defense.threat_boss > 0 or #d.defense.threat_trash > 0 or
		#d.defense.threat_stormvermin > 0
	...
	if not ret and not enemy_threat and auto_heal_enabled then
		...
		if not has_dove or has_draught then
			...
			local is_best_healer                = best_healer == self_unit and
				(not best_heal_target or best_heal_target == self_unit)
				...
				elseif status_ext:is_hanging_from_hook() then
					...
					ally_is_human and ally_can_accept_draught and can_give_healing_to_other and
						...
						bot_settings.menu_settings.pass_draughts_to_humans == "Always" or -- Always
							...
							(bot_settings.menu_settings.pass_draughts_to_humans_forced and is_best_human_to_receive_draught) or
							-- Always on hotkey
							(
					...
					--
					if self_has_healshare and
						((ally_health_percent < WANTS_TO_HEAL_THRESHOLD and deathwish_enabled) or ally_is_wounded
						) and best_healer == self_unit and healer_override and self_hp_compare < 0.999 then
					...
					elseif ally_unit == best_dove_target then
						...
						utility      = 25
					elseif can_heal_other and
						((ally_health_percent < WANTS_TO_HEAL_THRESHOLD and not heal_only_if_wounded) or ally_is_wounded) and
						best_healer == self_unit then
						-- heal ally
					...
					elseif pass_draught_to_human then
						...
						utility = 9
					elseif ally_is_bot and can_give_healing_to_other and ally_can_accept_draught and
						(ally_hp_compare < self_hp_compare
						) and (ally_health_percent < WANTS_TO_GIVE_HEAL_TO_OTHER or ally_is_wounded) and not self_has_healshare then
					...
					elseif ally_is_bot and can_give_healing_to_other and ally_can_accept_draught and ally_has_healshare and
						not self_has_healshare then
...
Mods.hook.set(mod_name, "PlayerBotBase._select_ally_by_utility", function(func, self, unit, blackboard, breed, t)
	...
	if no_manual_heal_optimization then
		...
		if selected_unit and ally_hp_percent < 1 and not utils:get_unit_data(unit, "disabled") then
			...
			for _, owner in pairs(bots) do
				...
				if not utils:get_unit_data(unit, "disabled") and can_perform_heal(unit, unit == selected_unit) and
					ally_distance < best_distance then
	...
	local override_ally_need = ally_unit and ("in_need_of_heal" == in_need_type) and
		(manual_heal_only or bot_heal_info.player_unit)
	...
	if not override_ally_need and ally_unit and ("can_accept_heal_item" == in_need_type) and manual_heal_only and
		...
		if our_health_extension and ally_health_extension and our_buff_extension and ally_buff_extension and
			our_status_extension and ally_status_extension then
	...
	if override_ally_need then
		...
		local can_give_grenade_to_other = grenade_slot_data and
			inventory_extn:get_item_template(grenade_slot_data).can_give_other
			...
			if AiUtils.unit_alive(human_player_unit) then
				...
				if allowed_follow_path then
					...
					if allowed_aid_path then
						...
						--
						if can_give_healing_to_other and (ally_has_healshare or pass_draught_to_human) and
							not human_inventory_extn:get_slot_data("slot_healthkit") then
	...
	if get(me.SETTINGS.FILE_ENABLED) then
		...
		for unit, data in pairs(self._bot_ai_data) do
			...
			if blackboard.allowed_to_take_health_pickup then
				...
				local follow_unit = data.follow_unit
				local out_of_range = follow_unit and
					(Vector3.distance_squared(POSITION_LOOKUP[follow_unit], pickup_pos) > MAX_PICKUP_RANGE_SQ)
	...
	if get(me.SETTINGS.KEEP_TOMES) then
		...
		for unit, _ in pairs(self._bot_ai_data) do
			...
			if health_extension and status_extension and health_slot_data ~= nil then
				local item = inventory_ext.get_item_template(inventory_ext, health_slot_data)
				if item.name == "wpn_side_objective_tome_01" and blackboard.allowed_to_take_health_pickup and
					not (blackboard.health_pickup == pinged_heal) then
					...
					local pickup_settings = pickup_extension and pickup_extension:get_pickup_settings()
					if (current_health <= 0.5 or status_extension.wounded) and
						(pickup_settings.item_name == "potion_healing_draught_01") then
						healthy = false
					elseif (current_health <= 0.2 or status_extension.wounded) and
						(pickup_settings.item_name == "healthkit_first_aid_kit_01") then
						healthy = false
					end
					if healthy then
				...
				if unit ~= need_next_bot then
					...
					if health_extension and inventory_ext and status_extension then
						...
						if health_slot_data ~= nil then
							local item = inventory_ext.get_item_template(inventory_ext, health_slot_data)
							if item.name == "wpn_side_objective_tome_01" and
								Vector3.distance(POSITION_LOOKUP[unit], POSITION_LOOKUP[need_next_bot_pickup]) < 15 then
								if not status_extension.is_knocked_down(status_extension) then
									current_health = health_extension.current_health_percent(health_extension)
								end
...
me.generate_points = function(ai_bot_group_system, player)
	...
	local disallowed_at_pos, current_mapping = ai_bot_group_system:_selected_unit_is_in_disallowed_nav_tag_volume(nav_world
		, player_pos)
	if disallowed_at_pos then
		local origin_point = ai_bot_group_system:_find_origin(nav_world, player_unit)

		points = ai_bot_group_system:_find_destination_points_outside_volume(nav_world, player_pos, current_mapping,
			origin_point, num_bots)
	else
		...
		local forward_angle_offset   = Vector3.flat_angle(Vector3(1, 0, 0), Vector3.normalize(Vector3.flat(camera_forward))) +
			math.pi
...
Mods.hook.set(mod_name, "AIBotGroupSystem._assign_destination_points",
	function(func, self, bot_ai_data, points, follow_unit, follow_unit_table)
		...
		local may_follow_player       = player_status_extension and
			not (player_status_extension:is_dead() or player_status_extension:is_ready_for_assisted_respawn())
		...
		if (follow_unit and may_follow_player) or (follow_unit_table and table.has_item(follow_unit_table, player_unit)) then
			...
			local player_moved         = player_position and old_follow_position[player_unit] and
				Vector3.length(old_follow_position[player_unit]:unbox() - player_position) > 0.001
			local repositioning_denied = not player_moved and
				(not slot_name or slot_name == "slot_healthkit" or slot_name == "slot_potion" or slot_name == "slot_grenade")
...
Mods.hook.set(mod_name, "SpawnManager._update_bot_spawns", function(func, self, dt, t)
	...
	local delta, humans, bots = self._update_available_profiles(self, profile_synchronizer, available_profile_order,
		available_profiles)
	...
	local allowed_bots = math.min(NUM_PLAYERS - humans,
		(not script_data.ai_bots_disabled or 0) and (script_data.cap_num_bots or NUM_PLAYERS))
	...
	if 0 < bot_delta then
		...
		while bots_spawned < bot_delta do
			...
			if owner_type == "available" then
				...
				local bot_player = player_manager.add_bot_player(player_manager, SPProfiles[profile_index].display_name,
					local_peer_id, "default", profile_index, local_player_id)
				local is_initial_spawn, status_slot_index = self._assign_status_slot(self, local_peer_id, local_player_id,
					profile_index)
		...
		for _, bot_player in ipairs(self._spawn_list) do
			...
			if player_manager.player(player_manager, bot_peer_id, bot_local_player_id) == bot_player then
				...
				if status.health_state ~= "dead" and status.health_state ~= "respawn" and status.health_state ~= "respawning" then
					...
					bot_player.spawn(bot_player, position, rotation, is_initial_spawn, ammo.slot_melee, ammo.slot_ranged,
						consumables[CONSUMABLE_SLOTS[1]], consumables[CONSUMABLE_SLOTS[2]], consumables[CONSUMABLE_SLOTS[3]])
				end
...
Mods.hook.set(mod_name, "BTBotTeleportToAllyAction.run", function(func, self, unit, blackboard, t)
	...
	elseif state == "a_star_search" and GwNavAStar.processing_finished(a_star) then
		if GwNavAStar.path_found(a_star) and GwNavAStar.path_distance(a_star) < MAX_ALLOWED_TELEPORT_DISTANCE and
			0 < GwNavAStar.node_count(a_star) then
...
Mods.hook.set(mod_name, "LinkerTransportationExtension._link_transported_unit",
	function(func, self, unit_to_link, teleport_on_enter)
...
Mods.hook.set(mod_name, "LinkerTransportationExtension._unlink_transported_unit",
	function(func, self, unit_to_unlink)
	...
	if player then
		if not player.remote then
			...
			if timed_block and procced and Unit.alive(attacking_unit) then
				...
				network_manager.network_transmit:send_rpc_server("rpc_attack_hit", damage_source_id, unit_id, hit_unit_id,
					attack_template_id, hit_zone_id, attack_direction, NetworkLookup.attack_damage_values["n/a"],
					NetworkLookup.hit_ragdoll_actors["n/a"], backstab_multiplier, hawkeye_multiplier)
			end
			...
			if (procced or script_data.debug_legendary_traits) and Unit.alive(attacking_unit) and
				ScriptUnit.has_extension(attacking_unit, "buff_system") and breed then
...
Mods.hook.set(mod_name, "GenericUnitDamageExtension.add_damage",
	function(func, self, attacker_unit, damage_amount, hit_zone_name, damage_type, damage_direction, damage_source_name,
	         hit_ragdoll_actor, damaging_unit)

		if not get(me.SETTINGS.FILE_ENABLED) then
			return func(self, attacker_unit, damage_amount, hit_zone_name, damage_type, damage_direction, damage_source_name,
				hit_ragdoll_actor, damaging_unit)
		end
		...
		if not add_damage_tracker[self_unit] then
			local self_current_health = (self_health_extension and self_health_extension.health - self_health_extension.damage) or
				false
		...
		local grenade_damage = damage_source_name == "grenade_frag_01" or damage_source_name == "grenade_frag_02" or
			damage_source_name == "grenade_fire_01" or damage_source_name == "grenade_fire_02"
		...
		local attacker_is_boss = attacker_breed and
			(attacker_breed.name == "skaven_rat_ogre" or attacker_breed.name == "skaven_storm_vermin_champion")

		local valid_aoe_attacker = (
			attacker_area_damage_extension and not attacker_area_damage_extension.owner_player and
				attacker_area_damage_extension.damage_source and attacker_area_damage_extension.damage_source ~= "n/a") or false
		...
		if self_health_extension and self_is_alive then
			if self_owner then -- players / bots
				if attacker_is_valid and not self_is_disabled and not last_one_standing then
					if self_owner.bot_player then -- enemies damage players
						...
						local scaler_damage = (attacker_is_boss and (self_effective_damage * scaler_data.damage_taken_boss_factor)) or
							self_effective_damage
						...
						if self_is_server then -- server only is intentional
							...
							local scaler_damage                      = (
								attacker_is_boss and (self_effective_damage * scaler_data.damage_taken_boss_factor)) or self_effective_damage
							local buffer_overflow                    = math.max(scaler_data.damage_taken_buffer + scaler_damage -
								scaler_data.damage_taken_buffer_size, 0)
							local absorbed                           = scaler_damage - buffer_overflow
							local debt_increment                     = (
								absorbed * scaler_data.damage_taken_buffer_debt_leak_factor + buffer_overflow) /
								scaler_data.damage_taken_human_bot_ratio / (full_human_count ^ 0.25)
							...
							scaler_data.damage_taken_buffer          = math.min(scaler_data.damage_taken_buffer + scaler_damage,
								scaler_data.damage_taken_buffer_size)
						end
					end
			...
			elseif self_breed then -- players damage enemies
				local attacker_type         = (attacker_owner and ((attacker_owner.bot_player and "bot") or "human")) or
					(attacker_breed and "enemy") or "unknown"
				local offense_scaler_damage = (self_breed.name == "skaven_rat_ogre" and self_effective_damage * 0.333) or
					(self_breed.name == "skaven_storm_vermin_champion" and self_effective_damage * 0.78) or self_effective_damage
				...
				if attacker_type == "human" and human_count > 0 then
					if not add_damage_weapon_factor[attacker_unit] then
						...
						add_damage_weapon_factor[attacker_unit] = bot_settings.lookup.token_damage_balancing[weapon_type] or
							bot_settings.lookup.token_damage_balancing.default
					end
				...
				elseif attacker_type == "bot" and bot_count > 0 then
					...
					if d then
						...
						if ob_from_human then
							...
							scaler_data:use_offense_tokens((damage_weight / bot_count) * offense_scaler_damage * weapon_factor *
								token_usage_boost)
						end
				...
				if self_breed.special and not scaler_data.specials_lookup.initiative[self_unit] and
					(attacker_type == "human" or attacker_type == "bot") then
				...
				if Unit.alive(self_unit) and attacker_unit == self_unit and not attacker_owner and
					not ScriptUnit.has_extension(self_unit, "door_system") and self_effective_damage > 0 and
					bot_settings.menu_settings.help_with_breakables_objectives then
					local target_health_extension = ScriptUnit.extension(self_unit, "health_system")
					if target_health_extension then
						if target_health_extension:is_alive() and
							target_health_extension:current_health_percent() >= bot_settings.menu_settings.breakables_objectives_threshold then
...
Mods.hook.set(mod_name, "DamageUtils.heal_network", function(func, healed_unit, healer_unit, heal_amount, heal_type)
	...
	if healed_unit == local_player_unit and current_damage and heal_type == "proc" then
		...
		bot_settings.scaler_data.damage_taken_buffer = math.max(bot_settings.scaler_data.damage_taken_buffer -
			effective_heal_amount, 0)
...
Mods.hook.set(mod_name, "Staggers.ai_stagger",
	function(func, attack_template, attacker_unit, hit_unit, hit_zone_name, attack_direction, t)
		...
		if target_breed and target_alive and self_owner then
			if not stagger_weapon_factor[self_unit] then
				local weapon_type, weapon_unit, weapon_template = get_equipped_weapon_info(self_unit)
				local lookup_table = (attack_is_push and bot_settings.lookup.token_push_balancing) or
					bot_settings.lookup.token_stagger_balancing
			...
			if self_owner.bot_player and bot_count > 0 then
				if attack_is_push then
					...
					bot_settings.scaler_data:use_offense_tokens((bot_settings.scaler_data.stagger_bot_push_weight / bot_count) *
						difficulty_factor * weapon_factor)
				else
					...
					bot_settings.scaler_data:use_offense_tokens((bot_settings.scaler_data.stagger_weight / bot_count) *
						difficulty_factor * weapon_factor)
				end
			...
			elseif human_count > 0 then
				...
				bot_settings.scaler_data:add_offense_tokens((bot_settings.scaler_data.stagger_weight / human_count) *
					difficulty_factor * weapon_factor)
				if human_count > 0 then
...
Mods.hook.set(mod_name, "ConflictDirector.register_unit_killed",
	function(func, self, unit, blackboard, killer_unit, killing_blow)
		...
		if killed_breed and self_owner then
			...
			if self_owner.bot_player then
				...
				if special_kill then
					local assist_by_human = bot_settings.scaler_data.specials_pinged_by_human[killed_unit] or
						bot_settings.scaler_data.specials_damaged_by_human[killed_unit]
...
local when_level_starts = function()
	...
	scaler_data.damage_taken_debt_trasfer_min       = 37.5 * 0.5 * scaler_data.damage_taken_debt_trasfer_factor *
		scaler_data:get_difficulty_modifier_damage()
	scaler_data.damage_taken_debt_trasfer_max       = 37.5 * 2.0 * scaler_data.damage_taken_debt_trasfer_factor *
		scaler_data:get_difficulty_modifier_damage()
