...
--[[
	===Change Log===
Version 20221021 - Different Bots Experimental Branch by VernonKun
...
* Forced regroup now based on proximity of teammates including bots
...
* Bots move towards ogre and Krench when the current position is bad for dodging
* Bots now move away from nearby enemies when using certain ranged attacks (copied from VT2 vanilla bots)
* Tweak bot performance scaling
* Increase bots' ambient shooting distance
...
* Control bot dodge direction, especially during kiting (should be partially done because dodging tries to align with original move direction)
...
{
	--edit start--
	trash_SV_detection_distance = 25,
	--edit end--
	assigned_enemies = {},
	...
	lookup = 
	{
		...
		ranged_keep_distance_breed_override = {
		...
		{
			default				= 1,
			es_1h_mace_shield	= 1,	--1.4
			es_1h_sword_shield	= 1,	--1.2
			dr_1h_hammer_shield	= 1,	--1.4
			dr_1h_axe_shield	= 1,	--1.2
		},
		
		token_stagger_balancing = 
		{
			default				= 1,
			bw_staff_geiser		= 1,	--0.3
			bw_staff_beam		= 1,	--0.5
			ww_hagbane			= 0.25,	--1		--0.75		-- Vernon: hagbane should do enough damage with low effort already
		},
		--edit end--
	...
	scaler_data = 
	{
		...
		--edit start--
		stagger_weight = 1,	--0.7
		stagger_bot_push_weight = 1,		--0.15
		damage_weight = 1,	--1.667
		grenade_damage_weight = 0.25,	--0.833
		kill_weight = 0,
		
		offense_token_tick = 0.333,
		offense_token_tick_objective = 10,	--5
		offense_tokens = 0,
		offense_token_passive_cap = 100,	--75	--100	-- Vernon: this is also the initial token amount and for general scaling
		offense_token_passive_cap_objective = 200,	--175
		offense_ranged_range = 25,	--20
		offense_max_delay_ranged = 1.5,
		offense_ranged_scaling_start = 50,
		offense_ranged_scaling_stop = 200,	--300
		offense_time_pool = 0,
	...
	dodge = 
	{
		recalculate_time = 0.125,	--0.2		--0.125
		recalculate_time_random = 0.05,
	},
}
...
local d_data_update = function(d)
	...
	local medium_radius		= 7		--bot_settings.trash_SV_detection_distance * (7/20)
	local near_radius		= 5		--bot_settings.trash_SV_detection_distance * (5/20)
	local melee_radius		= 3.5	--bot_settings.trash_SV_detection_distance * (3.5/20)
	...
	-- Vernon TODO: possibly shields can use a larger angle since the push is in all directions
	-- d.defense.is_surrounded = false
	...
	local slot_extension = ScriptUnit.extension(d.unit, "ai_slot_system")
	local num_enabled_slots = (slot_extension and (slot_extension.total_slots_count - slot_extension.disabled_slots_count)) or 0
	
	local distance_sq_to_followed_target = Vector3.distance_squared(d.position, d.follow_target_position)
	
	local check_being_surrounded = (d.defense.check_surrounded_timer < d.current_time) and (num_enabled_slots > 4) and (distance_sq_to_followed_target > 1)
	
	if check_being_surrounded then
		d.defense.check_surrounded_timer = d.current_time + 0.15 + 0.05 * math.random()
	end
	...
	for _,loop_unit in pairs(d.enemies.medium) do
		...
		if check_being_surrounded then
			local loop_offset_x = POSITION_LOOKUP[loop_unit].x - d.position.x
			local loop_offset_y = POSITION_LOOKUP[loop_unit].y - d.position.y
	...
	-- Vernon: check if the bot is surrounded by enemies
	if check_being_surrounded then
		d.defense.is_surrounded = (min_enemy_angle <= max_enemy_angle) and (surrounded_check_angle_radian < max_enemy_angle - min_enemy_angle)
	elseif d.defense.check_surrounded_timer + 5 < d.current_time then
		d.defense.is_surrounded = false
	end
...
-- Vernon: bots move towards ogre / Krench of current position is bad for dodging
...
Mods.hook.set(mod_name, "PlayerBotBase._update_target_enemy", function(func, self, dt, t)
	...
	local offense_scaler_melee_multiplier = math.clamp((bot_settings.scaler_data.offense_tokens / offense_token_passive_cap),0,cap_melee)
	--edit end--
...
--]]
	...
	if (num_attacking_sv_overhead > 0 or num_attacking_sv_sweep > 0 or num_threat_sv_push > 0) and d.defense.dodge_count_left and d.defense.dodge_count_left >= -1
		and ((not d.defense.calculate_dodge_update_time) or d.defense.calculate_dodge_update_time < d.current_time)
		and not d.token.dodge and not d.defense.is_dodging and d.defense.can_dodge and d.defense.on_ground then
		--
		-- d.token.dodge = true
		
		d.defense.calculate_dodge_update_time = d.current_time + bot_settings.dodge.recalculate_time + bot_settings.dodge.recalculate_time_random * math.random()
	...
	if not dodged_sv and not dodge_failed and ((not d.defense.calculate_dodge_update_time) or d.defense.calculate_dodge_update_time < d.current_time)
		and not d.token.dodge and not d.defense.is_dodging and d.defense.can_dodge and d.defense.on_ground then
		--
	...
	if d.defense.calculate_dodge_update_time and 5 < d.current_time - d.defense.calculate_dodge_update_time then
		d.defense.calculate_dodge_update_time = nil
	end
...
local disengage_above_nav = 0.75	--3
local disengage_below_nav = 0.75	--3
local function check_angle_disengage(nav_world, traverse_logic, start_pos, start_direction, angle, distance)
...
local function get_disengage_pos(nav_world, traverse_logic, start_pos, disengage_vector, move_distance)
	local disengage_direction = Vector3.normalize(Vector3.flat(disengage_vector))
	local success, pos = check_angle_disengage(nav_world, traverse_logic, start_pos, disengage_direction, 0, move_distance)
	...
	-- local angle_inc = (math.pi / 3) / subdivisions_per_side

	for i = 1, subdivisions_per_side, 1 do
		local angle = angle_inc * i
		success, pos = check_angle_disengage(nav_world, traverse_logic, start_pos, disengage_direction, angle, move_distance)
...
local function get_disengage_vector(current_pos, enemy_unit, keep_distance, keep_distance_sq)
	if is_unit_alive(enemy_unit) then
		...
		if bot_settings.lookup.ranged_keep_distance_breed_override[breed_name] then
			keep_distance = bot_settings.lookup.ranged_keep_distance_breed_override[breed_name]
...
-- Vernon TODO: make smaller steps but update more frequently, and check pathing each step
local update_disengage_position_ranged = function (d, blackboard, t, keep_distance, move_towards_ally, ally_distance_threshold)
...
--]]
	...
	for _,loop_unit in pairs(d.enemies.near) do
		local target_unit = blackboard.shoot and blackboard.shoot.target_unit
		local result = nil
		if target_unit and target_unit ~= loop_unit then
			result = get_disengage_vector(self_position, loop_unit, keep_distance, keep_distance_sq)
		end
	...
	-- Vernon TODO: maybe make bots move away from ogre if targeted by it?
			...
			if loop_unit ~= d.unit then
				...
				if enabled then
					...
					if loop_distance and loop_distance < ally_distance_threshold then
						...
						if result then
							num_close_targets = num_close_targets + 2
							disengage_vector = disengage_vector - result * 2
						end
					end
				end
			end
	...
	if num_close_targets > 0 then
		...
		-- disengage_vector = Vector3.normalize(disengage_vector)
		...
		if nav_world and traverse_logic then
			disengage_position = get_disengage_pos(nav_world, traverse_logic, self_position, disengage_vector, 2)
		end
	end
	...
	if disengage_position then
		...
		local interval = 0.3 + 0.2 * math.random()
		shoot_bb.disengage_update_time = t + interval
	end
end

local update_disengage_position_melee = function (d, blackboard, t, keep_distance, move_towards_ally, ally_distance_threshold)
	local first_person_ext = blackboard.first_person_extension
	local self_position = first_person_ext:current_position()
...
Mods.hook.set(mod_name, "BTBotMeleeAction._update_melee", function(func, self, unit, blackboard, dt, t)
	...
	local target_unit				= breakable_target or d.melee.target_unit
	local melee_bb					= blackboard.melee
	...
	if breakable_target then
		self_aim_pos		= self:_aim_position(target_unit, blackboard)
		target_breed		= nil
		target_pos			= Unit.local_position(breakable_target, 0)
	...
	local possibly_do_not_engage = {
		...
		skaven_rat_ogre = true,		-- Vernon TODO: maybe set to nil because stamina hardly matters in melee ogre decisions
	...
	if d.defense.is_surrounded then
		if not melee_bb.disengage_update_time then
			melee_bb.disengage_update_time = 0
		end
		
		if melee_bb.disengage_update_time < t then
			local keep_distance = 7		-- Vernon: surrounded check is done with medium radius = 7
			local move_towards_ally = not d.defense.is_boss_target
			local ally_distance_threshold = 3.5
			
			update_disengage_position_melee(d, blackboard, t, keep_distance, move_towards_ally, ally_distance_threshold)
	...
	local in_melee_range		= (breakable_target and target_distance <= MAX_MELEE_RANGE) or target_distance < melee_range
	-- Vernon TODO: check engage_range_max and engage_range_min
	...
	if (in_melee_range and not defending and d.melee_attack_allowed) and (breakable_target or not d.regroup.enabled or (d.regroup.enabled and target_in_front and target_breed.name ~= "skaven_rat_ogre")) and not DEBUG_NO_MELEE_ATTACKS then
...
-- Vernon: should now handled by generic positioning??
-- Vernon TODO: still need to work on flee pathing when ogre is near downed teammate
Mods.hook.set(mod_name, "PlayerBotBase._update_rat_ogre_cover", function(func, self)
	...
	if is_ogre_target and ally_disabled then
		return func(self)
	else
...
local function check_angle_engage(nav_world, target_position, start_direction, angle, distance)
	local direction = Quaternion.rotate(Quaternion(Vector3.up(), angle), start_direction)
...
local function get_engage_pos(nav_world, target_unit_pos, engage_from, melee_distance)
	...
	local success, pos = check_angle_engage(nav_world, target_unit_pos, start_direction, 0, melee_distance)
	...
	for i = 1, subdivisions_per_side, 1 do
		local angle = angle_inc*i
		success, pos = check_angle_engage(nav_world, target_unit_pos, start_direction, angle, melee_distance)
	...
	success, pos = check_angle_engage(nav_world, target_unit_pos, start_direction, math.pi, melee_distance)
...
Mods.hook.set(mod_name, "BTBotMeleeAction._update_engage_position", function(func, self, unit, blackboard, dt, t)
	...
	if engage_position then
		...
		melee_bb.engage_update_time = t + 0.175 + 0.025 * math.random()	--0.2
	end
...
Mods.hook.set(mod_name, "BTBotShootAction._aim", function(func, self, unit, blackboard, dt, t)
	...
	if d and d.settings.better_melee and d.settings.bot_file_enabled then
		...
		if keep_distance and shoot_bb.disengage_update_time < t then
			update_disengage_position_ranged(d, blackboard, t, keep_distance, move_towards_ally, ally_distance_threshold)
		else
	...
	if d and d.settings.better_melee and d.settings.bot_file_enabled then
		if (not d.defense.calculate_dodge_update_time) or d.defense.calculate_dodge_update_time < d.current_time then
			local calculate_dodge = false
...
-- Vernon: edit stagger offense tokens calculation
...
Mods.hook.set(mod_name, "Staggers.ai_stagger", function (func, attack_template, attacker_unit, hit_unit, hit_zone_name, attack_direction, t)
	...
	if target_breed and target_alive and self_owner then
		...
		-- local breed_not_staggered = false
		local pre_stagger_ranking = 0
		local blackboard = Unit.get_data(target_unit, "blackboard")
		if blackboard then
			-- breed_not_staggered = (not blackboard.stagger) or (blackboard.stagger == 0)
			local pre_stagger_type = blackboard.stagger_type or 0
			pre_stagger_ranking = stagger_ranking[pre_stagger_type] or 0
		end
		...
		local stagger_ranking = stagger_ranking[stagger_type] or 0
		...
		if self_owner.bot_player and bot_count > 0 then
			if pre_stagger_ranking < stagger_ranking then
