...
local weapon_anim_replacement = {
	[5] = { --profile_index 5 -> Kruber
		wh_1h_falchions = { --original anim -> replacing anim
			["attack_swing_up"] = "attack_swing_right", --second light attack
			["attack_swing_charge_diagonal"] = "attack_swing_charge", --first heavy charge
			["attack_swing_heavy_left_diagonal"] = "attack_swing_heavy", --first charged attack
			["attack_swing_charge_diagonal_left"] = "attack_swing_charge_left", --second heavy charge
			["attack_swing_heavy_right_diagonal"] = "attack_swing_heavy_right", --second charged attack
		}
	}
}
...
--edit--
		if event then
			...
			local profile_index = self.actions.dummy and self.actions.dummy.status_extension.profile_id
			if profile_index and weapon_anim_replacement[profile_index] then
				local weapon_type = ItemMasterList[self.item_name].item_type
				local weapons_animation_events = weapon_anim_replacement[profile_index][weapon_type]
				local weapon_event_replacement = weapons_animation_events and weapons_animation_events[event]
				
				if weapon_event_replacement then
					Unit.animation_event(owner_unit, weapon_event_replacement)
				else
