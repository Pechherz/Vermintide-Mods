...
mod.create_options = function(self)
	...
	self.options[self.widget_settings.DWARF_ONE_HANDED_WEAPONS_POSITION.save] = self.get(self.widget_settings.DWARF_ONE_HANDED_WEAPONS_POSITION
		.save)
	self.options[self.widget_settings.WAYWATCHER_DUAL_WEAPONS_POSITION.save] = self.get(self.widget_settings.WAYWATCHER_DUAL_WEAPONS_POSITION
		.save)
	self.options[self.widget_settings.ONE_HANDED_WEAPONS_POSITION.save] = self.get(self.widget_settings.ONE_HANDED_WEAPONS_POSITION
		.save)
	...
	safe_pcall(function()
		if slot_name == "slot_melee" or slot_name == "slot_ranged" then
			...
			if table.contains(def.one_handed, item_data.item_type) then
				...
				local option_belt = 1
				local option_back = 2
				if _1h_weapon_position == option_belt and not dwarf_weapon or
					dwarf_weapon and dwarf_one_handed_weapon_position == option_d_belt then
					if not left then
						item_setting = def[item_data.item_type].right.belt
					else
						item_setting = def[item_data.item_type].left.belt
					end
				elseif _1h_weapon_position == option_back and not dwarf_weapon or
					dwarf_weapon and dwarf_one_handed_weapon_position == option_d_back then
					if not left then
						item_setting = def[item_data.item_type].right.back
					else
						item_setting = def[item_data.item_type].left.back
			...
			if profile_name then
				if not left then
					item_setting = profile_name and def[item_data.item_type][profile_name] and
						def[item_data.item_type][profile_name].right
					item_setting = item_setting or def[item_data.item_type] and def[item_data.item_type].right
				else
					item_setting = profile_name and def[item_data.item_type][profile_name] and
						def[item_data.item_type][profile_name].left
...
Mods.hook.set(mod_name, "MatchmakingManager.update", function(func, self, dt, t)
	...
	if mod.get(mod.widget_settings.ACTIVE.save) then
			...
			if player then
				...
				if player_unit ~= nil then
					...
					if ScriptUnit.has_extension(player_unit, "health_system") and ScriptUnit.has_extension(player_unit, "status_system") then
						...
						if is_alive and mod.is_not_loading() and mod.current.profile[player_unit] and
							not mod.current.equipment[player_unit] then
		...
		-- Options
		-- Dwarf weapons
		if mod.options[mod.widget_settings.DWARF_WEAPONS_POSITION.save] ~=
			mod.get(mod.widget_settings.DWARF_WEAPONS_POSITION.save) then
		...
		if mod.options[mod.widget_settings.DWARF_ONE_HANDED_WEAPONS_POSITION.save] ~=
			mod.get(mod.widget_settings.DWARF_ONE_HANDED_WEAPONS_POSITION.save) then
			for unit, name in pairs(mod.current.profile) do
				if name == "dwarf_ranger" then
					mod:delete_units(unit)
				end
			end
			mod.options[mod.widget_settings.DWARF_ONE_HANDED_WEAPONS_POSITION.save] = mod.get(mod.widget_settings.DWARF_ONE_HANDED_WEAPONS_POSITION
				.save)
		end
		-- Waywatcher dual weapons
		if mod.options[mod.widget_settings.WAYWATCHER_DUAL_WEAPONS_POSITION.save] ~=
			mod.get(mod.widget_settings.WAYWATCHER_DUAL_WEAPONS_POSITION.save) then
			for unit, name in pairs(mod.current.profile) do
				if name == "wood_elf" then
					mod:delete_units(unit)
				end
			end
			mod.options[mod.widget_settings.WAYWATCHER_DUAL_WEAPONS_POSITION.save] = mod.get(mod.widget_settings.WAYWATCHER_DUAL_WEAPONS_POSITION
				.save)
		end
		-- One-handed weapons
		if mod.options[mod.widget_settings.ONE_HANDED_WEAPONS_POSITION.save] ~=
			mod.get(mod.widget_settings.ONE_HANDED_WEAPONS_POSITION.save) then
			for unit, name in pairs(mod.current.profile) do
				mod:delete_units(unit)
			end
			mod.options[mod.widget_settings.ONE_HANDED_WEAPONS_POSITION.save] = mod.get(mod.widget_settings.ONE_HANDED_WEAPONS_POSITION
				.save)
		end
		-- Downscale big weapons
		if mod.options[mod.widget_settings.DOWNSCALE_BIG_WEAPONS.save] ~=
			mod.get(mod.widget_settings.DOWNSCALE_BIG_WEAPONS.save) then
			for unit, name in pairs(mod.current.profile) do
				mod:delete_units(unit)
			end
		...
		if player then
			...
			local third_person = third_person_mod and not third_person_mod.firstperson.value or
				mod:is_first_person_blocked(player.player_unit) or false
...
Mods.hook.set(mod_name, "InventoryPackageSynchronizer.set_inventory_list",
	function(func, self, profile_index, inventory_list, inventory_list_first_person)
		if mod.get(mod.widget_settings.ACTIVE.save) then
			local players = Managers.player:human_and_bot_players()
			for k, player in pairs(players) do
				local profile_synchronizer = Managers.state.network.profile_synchronizer
